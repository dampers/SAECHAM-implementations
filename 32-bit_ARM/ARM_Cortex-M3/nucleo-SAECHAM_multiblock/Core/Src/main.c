...
{
	char buffer[80];
	    int len = sprintf(buffer, "0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x 0x%04x\r\n",
	    		In_text[0], In_text[1], In_text[2], In_text[3], In_text[4], In_text[5], In_text[6], In_text[7]);
	    HAL_UART_Transmit(&huart2, (uint8_t*)buffer, len, HAL_MAX_DELAY);
}
